...
// Tu código acá:
document.querySelector('#createdBy').innerHTML += ' Willy'; 
...
function ToDo (description) { 
  ...
  this.complete = false ;  // porque no esta completo aun
...
function buildToDo(todo, index) {
  // Tu código acá:
  let toDoShell = document.createElement('div') ;  // creo el elemento div y lo guardo en variable
  toDoShell.className = 'toDoShell' // creo la clase todoshell y su nombre
  let toDoText = document.createElement('span') ; // al igual que antes, creo span
  toDoText.innerHTML = todo.description ; // creo propiedad
  toDoText.id = index // creo id 
  if (todo.complete) { // si existe la prop
  toDoText.classList.add ('completeText') } ; // agrego a la clase
  toDoShell.appendChild(toDoText) ; // agrego a ToDoText como hijo/apendice de todoShell
  return toDoShell ; } 
...
function buildToDos(toDos) {
  // Tu código acá:
  var array = toDos.map(function (elemento, index){
...
function displayToDos() {
  // Tu código acá:
  let toDoContainer = document.querySelector('#toDoContainer') ; //busco el id y lo guardo en variable
  toDoContainer.innerHTML = ''  ; // seteo el innerhtml com str vacio
  let Result = buildToDos(toDoItems) ; // llamo a la funcion y le paso el array como arg
  Result.map(function (elemento) { // itero sobre el array...
    toDoContainer.appendChild(elemento) ; // y voy agregando el elemento
  })
...
function addToDo() {
  // Tu código acá:
  let value = document.getElementById('toDoInput').value ; // busco id y guardo
  let myTodo = new ToDo(value) ;  // creo nuevo toDO pasandole valor de input
  toDoItems.push(myTodo) ; // agrego mi nuevo objeto creado
  let inpt = document.getElementById('toDoInput') ; 
  inpt.value = '' ; // steeo el valor de input como un str vacio
  displayToDos() // llamo a la fcn para actualizar 
...
// Tu código acá:
document.getElementById('addButton').addEventListener('click', addToDo) // creo un boton que cuando se le de click pase add to do
